...
// Written inline, so written with a tag: `io.doc`...`` is `parseDocument(text)`, and this
// example is about the document, so the document form is the one we want.

// ── A document has two halves ─────────────────────────────────────────────────

// Everything before `---` is the HEADER: schemas, metadata, definitions.
// Everything after is the DATA.
const doc = io.doc`~ version: 2
...
const multi = io.doc`~ $user:  {name: string, email: email}
