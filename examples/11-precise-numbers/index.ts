...
const money = parse('a: 0.1m, b: 0.2m, price: 19.99m, padded: 1.50m').toObject() as any;
...
const ids = parse('safe: 9007199254740991, beyond: 9007199254740993n').toObject() as any;
...
// wrong.
console.log('\n12    ->', typeof parse('v: 12').toObject().v);
console.log('12m   ->', parse('v: 12m').toObject().v.constructor.name);
console.log('12n   ->', typeof parse('v: 12n').toObject().v);

// ── It survives the round trip ────────────────────────────────────────────────

const src = 'total: 19.99m, id: 9007199254740993n';
console.log('\noriginal   :', src);
console.log('round trip :', stringify(parse(src)));
