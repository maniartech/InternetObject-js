 ...
 */
import io, { parseDocument, stringify } from '../../src/index';
...
const money = io`a: 0.1m, b: 0.2m, price: 19.99m, padded: 1.50m` as any;
...
const ids = io`safe: 9007199254740991, beyond: 9007199254740993n` as any;
...
// wrong.
console.log('\n12    ->', typeof io`v: 12`.v);
console.log('12m   ->', io`v: 12m`.v.constructor.name);
console.log('12n   ->', typeof io`v: 12n`.v);

// ── It survives the round trip ────────────────────────────────────────────────

// `src` is a variable, so this half uses the FUNCTION form -- a tag needs a literal.
