 ...
 *
 * Writes two files into the conformance corpus:
 *
...
const SUITE_BLURB: Record<string, string> = {
  ...
  schema: 'a schema definition string → the compiled member description. Matching is subset/contains, so a port is not held to the reference implementation\'s bookkeeping.',
...
rows.push('#');
rows.push('# GENERATED by the reference implementation tools/corpus/catalog.ts. Regenerate after adding cases:');
...
rows.push('#   kind      which comparator the file needs — see the reference implementation tools/corpus/runner.ts');
