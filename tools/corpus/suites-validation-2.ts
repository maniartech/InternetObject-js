 ...
 * once.
 *
 * The cases are CHOSEN by hand; their outcomes are OBSERVED by running the reference implementation. Coverage aim: every
...
const OUT = requireCorpusPath('validation');
...
const binary: Case[] = [
  ...
  { group: 'the `binary` TYPE NAME is not registered (spec describes it; the reference implementation has no BinaryDef)',
...
const objectsAndPaths: Case[] = [
  ...
  // or be the value of its first member. the reference implementation resolves it by what the braces contain, and the
...
const SUITES: Suite[] = [
  ['optionality', 'The optionality matrix — optional (?) and nullable (*) against present, absent and null',
   ['Validation · OPTIONALITY',
    'Authoritative: outcomes produced by running the reference implementation parse().',
  ...
  ['binary', 'Binary values, and the `binary` schema type that is not registered yet',
   ['Validation \u00b7 BINARY',
    ...
    'different matter: the specification describes it, the reference implementation registers no BinaryDef, and',
