...
/**
 * Generate a `schema/*.io` corpus suite, deriving every compiled shape by RUNNING the reference implementation.
 *
 ...
 *
 * The emitted `expected` is the SUBSET a case asserts, not everything the compiler carries. the reference implementation
 * puts `format: "auto"`, `encloser` and `escapeLines: false` on every string memberdef, and spells
 * `optional: false` where the corpus omits it; asserting those would be asserting the reference implementation's
...
export function emitSchemaSuite(path: string, spec: SchemaSuiteSpec): number {
  ...
  rows.push('# escapeLines) that is the reference implementation\'s business and not the format\'s.');
  rows.push('# GENERATED by the reference implementation tools/corpus/suites-schema.ts — edit the case table there.');
