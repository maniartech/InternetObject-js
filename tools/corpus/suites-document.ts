 ...
 * shape the whole document projects to. Those are exactly the areas the reference implementation's `tests/core/` covers
 * and the corpus did not: the suite was empty before 2026-08-22.
 *
 * The cases are CHOSEN by hand; their outcomes are OBSERVED by running the reference implementation. Choosing well is
...
const OUT = requireCorpusPath('document');
