 ...
 *
 * The cases are CHOSEN by hand; their outcomes are OBSERVED by running the reference implementation.
 */

const OUT = requireCorpusPath('parser');
...
const COMMON = 'GENERATED by the reference implementation tools/corpus/suites-parser.ts — edit the case table there.';
...
const SUITES: SuiteSpec[] = [
  {
    ...
    header: [
      'Parser · VALUES',
      'Authoritative: values produced by running the reference implementation parse() then doc.toObject().',
  ...
  },
  {
    ...
    header: [
      'Parser · KEYS',
      'Authoritative: outcomes produced by running the reference implementation parse() then doc.toObject().',
  ...
  },
  {
    ...
    header: [
      ...
      'Authoritative: codes produced by running the reference implementation parse() on each input.',
      'DESIGNATED CODES ONLY. the reference implementation THROWS most structural errors rather than accumulating',
