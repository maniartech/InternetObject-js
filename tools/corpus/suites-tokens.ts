...
/**
 * Generate `tokenizer/*.io` corpus suites, deriving every token stream by RUNNING io-js2.
 *
...
const OUT_DIR = '../io-test-cases/tokenizer';
...
const SUITES: TokSuite[] = [
  {
    ...
    header: [
      ...
      'Authoritative: token streams produced by running io-js2\'s Tokenizer on each input.',
      '',
      ...
      '',
      'See io-js2 ADR 0003 §2. These two rules replaced a rule-per-case, and are the pair a port',
...
function main(): void {
  ...
  for (const suite of SUITES) {
    ...
    rows.push('# GENERATED by io-js2 tools/corpus/suites-tokens.ts — edit the case table there.');
