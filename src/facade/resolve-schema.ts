 ...
 * - `schemaName` given but not found → throw `ErrorCodes.schemaNotFound` (never silent)
...
export function resolveSchema(defs: Definitions | undefined, schemaName?: string): Schema | undefined {
  ...
  if (schemaName) {
    ...
    if (!resolved) {
      throw new IOError(ErrorCodes.schemaNotFound, `Schema '${schemaName}' not found in definitions.`);
    }
    return resolved;
  }
