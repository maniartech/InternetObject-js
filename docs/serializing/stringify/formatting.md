...
Arrays containing objects expand to multiple lines with each item on its own line. However, **the object items themselves stay inline**.
...
When formatting documents with collections, each `~` item follows the same formatting rules:
