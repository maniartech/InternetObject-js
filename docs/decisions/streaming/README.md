...
ADRs are **non-normative**. They capture rationale, not rules.

> **Where these came from, and where the rules are now.** These records were written alongside an
> in-repo streaming protocol document (`src/streaming/specs/PROTOCOL.md`, June 2026). That document
> was superseded two days later by the streaming chapter of the specification proper,
> [`io-specs/streaming/`](../../../../io-specs/streaming/), which carries every decision below as
> normative text and is gated by the spec-example runner and the `streaming/` suites of the
> io-test-cases corpus. The prose was retired on 2026-08-23 (kept, outside the repo, in
> `.private/streaming-specs-superseded/`); the ADRs stayed because the *why* behind a rule is not
> something a specification restates, and a port that disagrees with a rule will want it. A
> `PROTOCOL §n` citation in any record below refers to the retired document; the same rule is in
> io-specs under the matching heading.
...
> [`docs/decisions/0001-defer-strict-validation-mode.md`](../0001-defer-strict-validation-mode.md).
